...
def gps_navigation_dashboard():
            ...
            if locations:
                ...
                with col1:
                    ...
                    ">
                        ...
                        <small style="color: {text_color}; opacity: 0.8;">
                            🎥 Real-time analysis from video feeds
                        </small>
                    </div>
                    """, unsafe_allow_html=True)
                ...
                with col2:
                    ...
                    <div style="background: #f8f9fa; padding: 20px; border-radius: 10px; margin: 10px 0; border-left: 4px solid #6c757d;">
                        <h4 style="margin: 0 0 10px 0;">📊 Traffic Distribution</h4>
                        <p style="margin: 5px 0;">🔴 High Traffic Zones: {high_traffic_zones}</p>
                        <p style="margin: 5px 0;">🟡 Medium Traffic Zones: {medium_traffic_zones}</p>
                        <p style="margin: 5px 0;">🟢 Low Traffic Zones: {low_traffic_zones}</p>
                        <small>Based on live video analysis</small>
                    </div>
                    """, unsafe_allow_html=True)
            
            else:
                st.info("🚦 Traffic analysis will appear here when video processing is active")
        
        else:
            st.warning("⚠️ Traffic intelligence temporarily unavailable")
    
    except Exception as e:
        st.info("🚦 Connecting to traffic analysis system...")
 

    
    # === NEW: LIVE CAMERA TRAFFIC STATUS DISPLAY ===
    st.markdown("---")
    st.subheader("📹 Live Camera Traffic Conditions (Using Your Videos)")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for i, camera in enumerate(camera_info):
                with col1 if i == 0 else col2:
                    if i < len(locations):
                        ...
                        ">
                            <h4 style="margin: 0 0 10px 0; color: {text_color};">
                                📹 {camera['name']} (Live Video Analysis)
                            </h4>
                            <p style="margin: 5px 0; color: {text_color};">
                                <strong>📍 Prototype Location:</strong> {camera['address']}
                            </p>
                            <p style="margin: 5px 0; color: {text_color};">
                                <strong>🚦 Traffic Status:</strong> {emoji} {status}
                            </p>
                            <p style="margin: 5px 0; color: {text_color};">
                                <strong>🚗 Live Vehicle Count:</strong> {vehicles}
                            </p>
                            <p style="margin: 5px 0; color: {text_color};">
                                <strong>⏱️ Smart Signal Time:</strong> {signal_time}s
                            </p>
                            <p style="margin: 5px 0; color: {text_color};">
                                <strong>⌛ Current Wait Time:</strong> {waiting_time}s
                            </p>
                            <small style="color: {text_color}; opacity: 0.8;">
                                🎥 Live YOLO detection from Video {camera['video_id']} (prototype data for any route)
                            </small>
                        </div>
                        """, unsafe_allow_html=True)
                        
                        # Add mini video preview
                        with st.expander(f"📹 Live Feed Preview - {camera['name']}", expanded=False):
                        ...
                        ">
                            <h4 style="margin: 0 0 10px 0;">📹 {camera['name']}</h4>
            ...
            st.error("❌ Unable to fetch live camera data from your videos")
            st.info("🔧 Make sure your backend server is running with video processing")
    
    except Exception as e:
        st.error(f"❌ Error fetching video analysis data: {str(e)}")
        st.info("📹 Your video cameras will show traffic data here when backend is running")
    ...
    if st.button("🗺️ Plan Route & Check Traffic", type="primary", key="plan_route_btn"):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                st.subheader("📹 Live Video Traffic Analysis (Prototype Data)")
                ...
                try:
                    ...
                    if video_locations:
                        st.info("🎥 **Prototype Note:** Using live video analysis data for route intelligence")
                            ...
                            with video_cols[i]:
                                if i < len(video_locations):
                                    ...
                                    <div style="background: {color}; padding: 12px; border-radius: 8px; 
                                        ...
                                        <h5 style="margin: 0;">📹 {camera['name']} {emoji}</h5>
                                        ...
                                        <small>🎥 Real-time YOLO analysis (represents route conditions)</small>
                                    </div>
                                    """, unsafe_allow_html=True)
                        
                except Exception as e:
                    st.info("📹 Live video analysis: Connect backend to see camera data")
                ...
                if road_route and road_route["coordinates"]:
                    ...
                    folium.PolyLine(
                        ...
                        opacity=0.9,
                        popup=f"🛣️ Route: {road_route['distance']:.1f} km | 🚦 {route_traffic_condition} Traffic | {traffic_message}"
                    ).add_to(m)
                    ...
                    <div style="background: {route_color}; color: white; padding: 10px; border-radius: 5px; margin: 10px 0; text-align: center;">
                        <strong>🛣️ Route Traffic Status: {route_traffic_condition}</strong><br>
                ...
                try:
                    ...
                    for i, vid_cam in enumerate(video_camera_coords):
                        if i < len(live_camera_locations):
                            ...
                            popup_html = f"""
                            <div style="width: 200px;">
                                <h4 style="color: {vid_cam_color};">📹 {vid_cam['name']}</h4>
                                <p><strong>🚦 Live Status:</strong> {status}</p>
                                <p><strong>🚗 Vehicles:</strong> {vehicles}</p>
                                <p><strong>⏱️ Signal:</strong> {signal_time}s</p>
                                <small>🎥 Live YOLO detection from your video</small>
                            </div>
                            """
                            ...
                            folium.Marker(
                                ...
                                tooltip=f"📹 {vid_cam['name']} - {status} Traffic",
                ...
                if transport_estimates:
                    ...
                    if 'route_traffic_condition' in locals():
                        ...
                        st.subheader(f"🚌 Transport Options (Adjusted for {route_traffic_condition} Traffic)")
...
def main():
    ...
    if user_role == "authority" and health_data and "ecocoin_system" in health_data:
        ...
        with col3:
            st.metric("🌱 Total CO₂ Saved", f"{ecocoin_data['total_co2_saved']} kg")
        
        with col4:
            generation_type = ecocoin_data.get("ecocoin_generation", "system_based")
            generation_display = "👤 User Trip Based" if generation_type == "user_trip_based" else "⚙️ System Based"
